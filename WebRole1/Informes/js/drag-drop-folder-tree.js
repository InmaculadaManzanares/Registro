...
************************************************************************************************************/	
	...
	function JSDragDropTree()
	{
		...
		//CAMBIO ESP
		var additionalAddSonRequestParameters = {};
		...
		//CAMIO ESP
		var nombreditinformarclick;
		var nombrebotoninformarclick;
		...
		// CAMBIO ESP
		this.addsonAllowed = true;
		this.currentlyActiveItem = false;
		this.filePathRenameItem = 'arboldragdrop_updateitem.aspx';
		this.filePathDeleteItem = 'arboldragdrop_deleteitem.aspx';
		// CAMBIO ESP
		this.filePathAddSon = 'arboldragdrop_addsonitem.aspx';
	...
	JSDragDropTree.prototype = {
		...
		,
		setFileNameAddSon : function(newFileName)
		{
			this.filePathAddSon = newFileName;
		}
		,
		...
		,
		setAdditionalAddSonRequestParameters : function(requestParameters)
		{
			this.additionalAddSonRequestParameters = requestParameters;
		}
		...
		,
		setAddsonAllowed : function(addsonAllowed)
		{   //CAMBIOS ESP 
			this.addsonAllowed = addsonAllowed;	
		}
		,
		setnombreeditinformarclick : function(nombreedit)
		{  //CAMBIOS ESP
		    this.nombreditinformarclick =nombreedit;
		}
		,
		setnombrebotoninformarclick : function(nombreboton)
		{  //CAMBIOS ESP
		    this.nombrebotoninformarclick =nombreboton;
		}
		...
		*/
		...
		,onclick : function (inputobj, e)
		{
		//CAMBIO ESP
		if(JSTreeObj.currentlyActiveItem)JSTreeObj.currentlyActiveItem.className = '';
			this.className = 'highlightedNodeItem';
			JSTreeObj.currentlyActiveItem = this;
		 var obj = document.getElementById(JSTreeObj.nombreditinformarclick);
		 //alert('Actualizar: ' + JSTreeObj.nombreditinformarclick + ' Valor: ' + this.parentNode.id);
           obj.value=this.parentNode.id;
           var obj2 = document.getElementById(JSTreeObj.nombrebotoninformarclick);
            if (obj2){
                obj2.click();   
            }
         //  __doPostBack(JSTreeObj.nombrebotoninformarclick,'');
		//alert('Nodo: ' + this.parentNode.id);
		...
		{
		//CAMBIO ESP
			var message = 'Pulse OK para confirmar el borrado del nodo ' + obj2.innerHTML;
			if(this.hasSubNodes(obj2.parentNode)) message = message + ' y de todos sus hijos';
			if(confirm(message)){
		...
		__deleteComplete : function(ajaxIndex,obj)
		{
			if(this.ajaxObjects[ajaxIndex].response!='OK'){
				alert('ERROR BORRANDO EL NODO: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
			}else{
		...
		{
			if(this.ajaxObjects[ajaxIndex].response!='OK'){
				alert('ERROR RENOMBRANDO EL NODO: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
			}
		}
		,
		...
		,
		addsonItem : function(obj1,obj2)
		{
			currentItemToEdit = obj2.parentNode;	// Reference to the <li> tag.
			var nuevohijo = document.createElement('LI');
			var textohijo = document.createElement('A');
			var nuevonombre = 'Nuevo Nodo hijo de ' + currentItemToEdit.id
			textohijo.setAttribute('href','#');
			textohijo.innerText =nuevonombre;
			nuevohijo.appendChild(textohijo);
			nuevohijo.id = 'nuevo'  + currentItemToEdit.id + '-' + Math.floor(Math.random()*1001);
			
			
			var img = document.createElement('IMG');
            img.src = this.imageFolder + this.plusImage;
            img.onclick = JSTreeObj.showHideNode;
            // Lo acabamos de crear no tiene ningun hijo
            img.style.visibility='hidden'

            textohijo.id = 'nodeATag' + nuevohijo.id.replace(/[^0-9]/gi,'');
			textohijo.onmousedown = JSTreeObj.initDrag;
            textohijo.onmousemove = JSTreeObj.moveDragableNodes;

            nuevohijo.insertBefore(img,textohijo);
            ...
            nuevohijo.insertBefore(folderImg,textohijo);			
					
			var noDelete = currentItemToEdit.getAttribute('noDelete');
			if(!noDelete)noDelete = currentItemToEdit.noDelete;
			var noRename = currentItemToEdit.getAttribute('noRename');
			if(!noRename)noRename = currentItemToEdit.noRename;		
			
			if(this.contextMenu){
					if(noRename=='true' && noDelete=='true'){}else{
						if(noDelete == 'true')this.contextMenu.attachToElement(textohijo,false,menuModelRenameOnly);
						else if(noRename == 'true')this.contextMenu.attachToElement(textohijo,false,menuModelDeleteOnly);
						else this.contextMenu.attachToElement(textohijo,false,menuModel);
					
					}
				}
				this.addEvent(nuevohijo,'contextmenu',this.highlightItem);
			var uls = currentItemToEdit.getElementsByTagName('UL');
                    if(uls.length>0){
						ul = uls[0];
						ul.style.display='block';
						...
						if(lis.length>0){	// Sub elements exists - drop dragable node before the first one
							ul.insertBefore(nuevohijo,lis[0]);	
						}else {	// No sub exists - use the appendChild method - This line should not be executed unless there's something wrong in the HTML, i.e empty <ul>
							ul.appendChild(nuevohijo);	
						}
					}else{
						var ul = document.createElement('UL');
						ul.style.display='block';
						currentItemToEdit.appendChild(ul);
						ul.appendChild(nuevohijo);
					}
					var img = currentItemToEdit.getElementsByTagName('IMG')[0];					
					img.style.visibility='visible';
					img.src = img.src.replace(JSTreeObj.plusImage,JSTreeObj.minusImage);					
	
			        this.__addsonItem_step2(nuevohijo)

		},
		__addsonItem_step2 : function(obj)
		{
			...
			JSTreeObj.ajaxObjects[ajaxIndex] = new sack();
			JSTreeObj.ajaxObjects[ajaxIndex].method = "GET";
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("fatherId", obj.parentNode.parentNode.id);
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("Name",  obj.value);
			JSTreeObj.__addAdditionalRequestParameters(JSTreeObj.ajaxObjects[ajaxIndex], JSTreeObj.additionalAddSonRequestParameters);			 
			JSTreeObj.ajaxObjects[ajaxIndex].requestFile = JSTreeObj.filePathAddSon;	// Specifying which file to get
			JSTreeObj.ajaxObjects[ajaxIndex].onCompletion = function() { JSTreeObj.__addsonComplete(ajaxIndex,obj); } ;	// Specify function that will be executed after file has been found
			JSTreeObj.ajaxObjects[ajaxIndex].onError = function() { JSTreeObj.__onErrorAjax(ajaxIndex,obj); } ;	
			JSTreeObj.ajaxObjects[ajaxIndex].runAJAX();		// Execute AJAX function				
			
			
		}
		,
		__onErrorAjax : function(ajaxIndex,obj)
		{
		    alert("Error en la llamada AJAX:" + this.ajaxObjects[ajaxIndex].response);
		}
		,
		__addsonComplete : function(ajaxIndex,obj)
		{
		    var respuesta = this.ajaxObjects[ajaxIndex].response.substring(0,2);
		    var idnuevo =this.ajaxObjects[ajaxIndex].response.replace(/[^0-9]/gi,'');
			if(respuesta!='OK'){
				alert('ERROR  EL NODO: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
				var parentRef = obj.parentNode.parentNode;
				obj.parentNode.removeChild(obj);
				this.__refreshDisplay(parentRef);
			}
		...
		initTree : function()
		{
			...
			/* Create context menu */
			if(this.deleteAllowed || this.renameAllowed || this.addsonAllowed){
				try{
					...
					if(this.addsonAllowed)menuModel.addItem(100,'Nuevo Hijo','','',false,'JSTreeObj.addsonItem');
					menuModel.init();	
					...
					//CAMBIO ESP
					var menuModelAddSonOnly = new DHTMLGoodies_menuModel();
					if(this.deleteAllowed)menuModelDeleteOnly.addItem(5,'Añade Hijo','','',false,'JSTreeObj.deleteItem');
					menuModelDeleteOnly.init();	
			...
			for(var no=0;no<menuItems.length;no++){
				...
				if(this.contextMenu){
					...
					//CAMBIO ESP
					var noAddSon = menuItems[no].getAttribute('noAddSon');
					if(!noAddSon)noAddSon = menuItems[no].noAddSon;
				...
				//CAMBIO ESP
				this.addEvent(aTag,'click',this.onclick);

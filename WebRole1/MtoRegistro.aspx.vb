    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Not Page.IsPostBack Then
                borra_campos()
                filtro()
                cargacombo_campos("SELECT  Email,   Nombre,  Situacion FROM  RegistroHorario", combo_campo)
                ...
                texttitulo.Text = "Registro Vacaciones/Bajas/Otros"
    ...
    Protected Sub CargaMatriz(ByVal cond_filtro As String)
        Try
            ...
            cadena = "SELECT   RegistroHorario.id ,     RegistroHorario.email, Usuarios.Nombre, empresas.Nombre AS empresa,  
                         centros.Nombre AS centro,  RegistroHorario.Fecha, RegistroHorario.oficina, RegistroHorario.Obs
FROM            Usuarios INNER JOIN
                         ...
                         empresas ON Usuarios.Empresa = empresas.Id where (oficina = 'VACACIONES' OR oficina = 'BAJA' or oficina = 'OTROS') "
            If cond_filtro <> "" Then
                Me.SqlDS.SelectCommand = cadena & " and " & cond_filtro & sql_ORDERBY & " RegistroHorario.id desc"
            Else
                Me.SqlDS.SelectCommand = cadena & sql_ORDERBY & " RegistroHorario.id desc "
            End If
    ...
    Public Sub filtro()
        ...
        If valor <> VACIO Then
            ...
            If campo = "Situacion" Then campo = " RegistroHorario.oficina "
    ...
    Protected Function Guardar(ByVal clave As String) As Boolean
        ...
        If txtid.Text = "" Then
            Try
                ...
                '        If x = 1 Then
                '            .add_valor_expr("fecha", Util.fechasql(txtfecha.Text, tipofecha.yyyyddMMmmss))
                '        Else
                '            .add_valor_expr("fecha", Util.fechasql(DateAdd(DateInterval.Day, x - 1, CDate(txtfecha.Text)), tipofecha.yyyyddMMmmss))
                '        End If
                '        .add_valor("oficina", cmbmotivo.Text)
                ...
                Dim fecha As Date
                fechaActual = CDate(txtfecha.Text)
                For x = 1 To CInt(txtdias.Text)


                    For t = 0 To 365
                        Dim esFinDeSemana As Boolean = (fechaActual.DayOfWeek = DayOfWeek.Saturday OrElse fechaActual.DayOfWeek = DayOfWeek.Sunday)
                        Dim esFestivo As Boolean = festivo(fechaActual.Date)
                        If esFinDeSemana Then
                            If sabadodomingo.Checked Then
                                Exit For
                            End If
                        Else
                            If esFestivo Then
                                If festivos.Checked Then
                                    Exit For
                                End If
                            Else
                                Exit For 'no es fin de semana ni fectivo
                            End If
                        End If
                    ...
                    With ins
                        .Tabla = "RegistroHorario"
                        .add_valor(CAMPO_EMAIL, cmbusuario.Text)
                        .add_valor_expr("fecha", Util.fechasql(fechaActual, tipofecha.yyyyddMMmmss))
                        .add_valor("oficina", cmbmotivo.Text)
                        .add_valor("obs", txtobs.Text)
            ...
            Try
                ...
                With upd
                    ...
                    If x = 1 Then
                        .add_valor_expr("fecha", Util.fechasql(txtfecha.Text, tipofecha.yyyyddMMmmss))
                    Else
                        .add_valor_expr("fecha", Util.fechasql(DateAdd(DateInterval.Day, x, CDate(txtfecha.Text)), tipofecha.yyyyddMMmmss))
                    End If
                    .add_valor("oficina", cmbmotivo.Text)
                    .add_valor("obs", txtobs.Text)
        ...
        Return resul
    End Function
    Private Function festivo(fecha As Date) As Boolean
        Dim con As ConexionSQL = New ConexionSQL(VariablesGlobales.CadenadeConexion(Application))
        If con.ejecuta1v_string("SELECT Festivos FROM MyDates WHERE (mydate = CONVERT(DATETIME, '" & fecha & "', 103))") = "Festivo" Then
            Return True
        Else
            Return False
        End If
    ...
    Protected Sub btnEdit_Click(sender As Object, e As EventArgs)
        ...
        For Each dr In dt.Rows
            ...
            If dr("fecha").ToString <> "" Then txtfecha.Text = CStr(dr("fecha"))
            If dr("oficina").ToString <> "" Then cmbmotivo.Text = CStr(dr("oficina"))
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs)
        If cmbmotivo.Text <> VACIO And cmbusuario.Text <> VACIO And IsDate(txtfecha.Text) Then
            If txtid.Text = "" Then
                If IsNumeric(txtdias.Text) = False Then
                    msj_alerta = "Días incorrectos."
                    mostraralerta()
                    activar_panel()
                End If
            End If
